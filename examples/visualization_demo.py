...
def create_random_walk_simulation(
    num_objects: int = 10, world_width: float = 1920, world_height: float = 1080
...
def run_visualization_demo(num_objects: int = 50, realtime: bool = True):
    ...
    while True:
        ...
        cv2.putText(
            frame_img, f"FPS: {fps:.1f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2
        )
...
def main():
    ...
    try:
        ...
        print(
            f"- random_walk_demo_{num_objects}objects_{num_frames}frames.mp4 (if OpenCV available)"
        )
