...
def example_basic_tracking():
    ...
    for frame_num in range(20):
        detections = create_synthetic_detections(frame_num, num_objects=2)  # Only 2 objects for simplicity
        
        tracked_objects = tracker.update(detections)
        
        print(f"Frame {frame_num:2d}: {len(detections)} detections -> {len(tracked_objects)} tracks")
...
def example_embedding_tracking():
    ...
    print(f"Tracker config: embedding_weight={config.embedding_weight}, "
    ...
    for frame_num in range(15):
        ...
        if frame_num % 5 == 0 and frame_num > 0:
            ...
            if emb_stats['ready']:
                print(f"  Embedding scaler: method={emb_stats['method']}, "
    ...
    try:
        ...
        for i, (track_id, data) in enumerate(track_data.items()):
            ...
            plt.plot(positions[:, 0], positions[:, 1], 'o-', 
                    color=colors[i], label=f'Track {track_id}', linewidth=2, markersize=4)
        ...
        plt.subplot(2, 2, 2)
        for i, (track_id, data) in enumerate(track_data.items()):
            plt.plot(data['frames'], data['ages'], 'o-', 
...
def example_visualization_capabilities():
    ...
    try:
        from swarmsort import (
            TrackingVisualizer, ObjectMotionSimulator, 
            create_demo_scenario, quick_visualize,
        ...
        if detection_sequences and track_sequences:
            ...
            print("Final frame: {} detections, {} tracks".format(
                len(detection_sequences[-1]), len(track_sequences[-1])
            ))
        
        print("For advanced visualization examples, run: python examples/visualization_demo.py [num_objects] [num_frames]")
