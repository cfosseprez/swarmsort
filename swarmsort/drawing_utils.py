...
class TrackingVisualizer:
    ...
    def __init__(self, config: Optional[VisualizationConfig] = None):
        ...
        if not MATPLOTLIB_AVAILABLE and not OPENCV_AVAILABLE:
            raise ImportError("Either matplotlib or opencv-python must be installed for visualization")
    
    def draw_frame_matplotlib(self, 
        ...
        for track in tracks:
            ...
            if self.config.show_trails and len(self.trail_history[track.id]) > 1:
                ...
                ax.plot(trail_points[:, 0], trail_points[:, 1], 
            ...
            if self.config.show_ids:
                ...
                ax.text(x + 10, y - 10, text, 
                       fontsize=8, color=color, weight='bold',
                       bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.7))
    
    def draw_frame_opencv(self, 
                         detections: List[Detection], 
                         tracks: List[TrackedObject],
                         frame_num: int,
        ...
        for track in tracks:
            ...
            if self.config.show_trails and len(self.trail_history[track.id]) > 1:
                ...
                for i in range(1, len(trail_points)):
                    ...
                    cv2.line(frame, tuple(trail_points[i-1]), tuple(trail_points[i]), 
                            trail_color, 1)
    ...
    def create_video_sequence(self,
        ...
        try:
            for frame_num, (detections, tracks) in enumerate(zip(detection_sequence, track_sequence)):
    ...
    def show_sequence_matplotlib(self,
                               detection_sequence: List[List[Detection]],
                               track_sequence: List[List[TrackedObject]],
                               interval: int = 100,
        ...
        def animate(frame_num):
            if frame_num < len(detection_sequence) and frame_num < len(track_sequence):
                self.draw_frame_matplotlib(
                    detection_sequence[frame_num], 
                    track_sequence[frame_num], 
                    frame_num, ax
                )
        
        anim = FuncAnimation(fig, animate, frames=len(detection_sequence),
...
def quick_visualize(detections: List[Detection], 
                   tracks: List[TrackedObject],
                   frame_num: int = 0,
                   method: str = "matplotlib") -> None:
