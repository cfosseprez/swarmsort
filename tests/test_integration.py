...
class TestCompleteTrackingScenarios:
    ...
    def test_single_object_tracking(self, basic_tracker, tracking_assertions):
        ...
        for i in range(10):
            detection = Detection(
                position=np.array([10.0 + i * 3, 20.0 + i * 2]),
                confidence=0.9,
                id=f"det_{i}"
            )
    ...
    def test_multiple_objects_tracking(self, basic_config, tracking_assertions):
        ...
        for i in range(8):
            detections = [
                ...
                Detection(position=np.array([80.0 - i, 50.0 + i]), confidence=0.85)  # Moving diagonally
            ]
...
class TestEmbeddingIntegration:
    ...
    def test_embedding_based_association(self, embedding_config):
        ...
        for i in range(8):
            ...
            detections = [
                Detection(
                    position=np.array([10.0 + i + np.random.randn() * 2, 20.0 + np.random.randn() * 2]),
                    confidence=0.9,
                    embedding=emb1 + np.random.randn(64) * 0.1  # Small variation
                ),
                Detection(
                    position=np.array([50.0 + i + np.random.randn() * 2, 30.0 + np.random.randn() * 2]),
                    confidence=0.8,
...
class TestConfigurationIntegration:
    ...
    def test_strict_vs_permissive_configs(self):
        ...
        noisy_detections = [
            ...
            Detection(position=np.array([15.0, 25.0]), confidence=0.4),  # Low confidence, close to first
        ]
    ...
    def test_reid_enabled_vs_disabled(self):
        ...
        reid_config = SwarmSortConfig(
            reid_enabled=True,
            reid_max_distance=80.0,
            reid_embedding_threshold=0.5,
            max_age=3
        )
        
        no_reid_config = SwarmSortConfig(
            reid_enabled=False,
            max_age=3
        )
...
class TestConfigurationPersistence:
    ...
    def test_config_to_dict_roundtrip(self):
        ...
        assert restored_config.min_consecutive_detections == original_config.min_consecutive_detections
