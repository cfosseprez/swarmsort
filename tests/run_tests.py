...
def run_linting():
    ...
    commands = [
        (
            [sys.executable, "-m", "black", "--check", "swarmsort/", "tests/", "examples/"],
            "Black formatting check",
        ),
        (
            [sys.executable, "-m", "isort", "--check-only", "swarmsort/", "tests/", "examples/"],
            "Import sorting check",
        ),
        ([sys.executable, "-m", "flake8", "swarmsort/", "tests/", "examples/"], "Flake8 linting"),
        (
            [sys.executable, "-m", "mypy", "swarmsort/", "--ignore-missing-imports"],
            "MyPy type checking",
        ),
    ]
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run in quick mode (less verbose, skip optional tests)"
    )
    parser.add_argument("--skip-slow", action="store_true", help="Skip slow tests")
    parser.add_argument("--skip-stress", action="store_true", help="Skip stress tests")
    parser.add_argument(
        "--benchmark-only", action="store_true", help="Only run benchmarks for performance tests"
    )
    ...
    if args.mode == "unit":
        ...
        success = run_all_tests(
            quick=args.quick, skip_slow=args.skip_slow, skip_stress=args.skip_stress
        )
    elif args.mode == "ci":
