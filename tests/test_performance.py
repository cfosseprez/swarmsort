...
class MemoryProfiler:
    ...
    def get_peak_usage(self) -> float:
        ...
        try:
            peak_rss = self.process.memory_info().peak_wss if hasattr(self.process.memory_info(), 'peak_wss') else 0
...
class TestNumbaFunctionPerformance:
    ...
    def test_embedding_distances_multi_history_performance(self, benchmark):
        ...
        result = benchmark(
            compute_embedding_distances_multi_history,
            det_embeddings, track_embeddings, track_counts, 'best_match'
        )
    ...
    def test_cost_matrix_performance(self, benchmark, matrix_size):
        ...
        track_positions = np.random.rand(n_track, 2).astype(np.float32) * 100
        max_distance = 50.0
        
        # Warm up JIT
        compute_cost_matrix_vectorized(det_positions, track_positions, max_distance)
        
        result = benchmark(
            compute_cost_matrix_vectorized,
            det_positions, track_positions, max_distance
        )
    ...
    def test_memory_efficiency(self):
        ...
        for frame in range(100):
            ...
            if frame % 20 == 19:
                ...
                assert abs(memory_increase) < 100, f"Memory usage changed by: {memory_increase:.2f}MB"
...
class TestScalabilityBenchmarks:
    ...
    def generate_scenario(self, num_objects: int, num_frames: int, 
                         embedding_dim: int = 128, noise_level: float = 1.0):
...
class TestMemoryBenchmarks:
    ...
    def test_track_history_memory_management(self):
        ...
        config = SwarmSortConfig(
            use_embeddings=True,
            max_embeddings_per_track=5,  # Limited history
            max_age=8
        )
        tracker = SwarmSort(config)
        ...
        for frame in range(100):
            ...
            if frame % 25 == 24:
                ...
                if frame > 50:  # After initial ramp-up
                    # Use absolute value to handle potential negative values due to GC
                    assert abs(memory_increase) < 150, f"Memory usage at frame {frame}: {memory_increase:.2f}MB"
...
def run_performance_comparison():
    ...
    configurations = [
        ...
        ("High Precision", SwarmSortConfig(use_embeddings=True, embedding_weight=0.6, min_consecutive_detections=4)),
        ("Fast", SwarmSortConfig(use_embeddings=False, max_distance=100, min_consecutive_detections=1))
    ]
