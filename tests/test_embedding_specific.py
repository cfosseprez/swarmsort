...
from swarmsort import SwarmSort, SwarmSortConfig, Detection, TrackedObject, EmbeddingDistanceScaler
from swarmsort.core import cosine_similarity_normalized, compute_embedding_distances_multi_history
...
class TestEmbeddingDistanceScaler:
    ...
    def test_scaler_initialization(self):
        ...
        # Custom scaler
        scaler2 = EmbeddingDistanceScaler(method="min_robustmax", update_rate=0.1, min_samples=50)
    ...
    def test_different_scaling_methods(self, scaling_method):
        ...
        scaler = EmbeddingDistanceScaler(method=scaling_method, min_samples=20)
    ...
    def test_online_adaptation(self):
        ...
        scaler = EmbeddingDistanceScaler(
            min_samples=20, update_rate=0.2  # High update rate for testing
        )
...
class TestEmbeddingBasedTracking:
    ...
    def test_embedding_association_accuracy(self, consistent_embeddings):
        ...
        for i in range(10):
            ...
            if i < 5:
                pos1, pos2 = np.array([10 + i * 10, 50], dtype=np.float32), np.array(
                    [100 - i * 10, 50], dtype=np.float32
                )
            else:
                pos1, pos2 = np.array([100 - (i - 5) * 10, 50], dtype=np.float32), np.array(
                    [10 + (i - 5) * 10, 50], dtype=np.float32
                )
    ...
    def test_embedding_distance_computation(self):
        ...
        track_embeddings = track_embeddings / np.linalg.norm(
            track_embeddings, axis=1, keepdims=True
        )
    ...
    def test_embedding_similarity_thresholds(self):
        ...
        # First detection
        det1 = Detection(
            position=np.array([10, 20], dtype=np.float32), confidence=0.9, embedding=emb1
        )
        tracker.update([det1])

        # Second detection at similar position but very different embedding
        det2 = Detection(
            position=np.array([12, 22], dtype=np.float32), confidence=0.9, embedding=emb2
        )
    ...
    def test_embedding_normalization(self):
        ...
        for i, embedding in enumerate(test_embeddings):
            detection = Detection(
                position=np.array([i * 20, 50]), confidence=0.9, embedding=embedding
            )
...
class TestEmbeddingEdgeCases:
    ...
    def test_very_high_dimensional_embeddings(self):
        ...
        for dim in dimensions:
            ...
            detection = Detection(position=np.array([10, 20]), confidence=0.9, embedding=embedding)
    ...
    def test_embedding_with_special_values(self):
        ...
        for i, embedding in enumerate(special_embeddings):
            detection = Detection(
                position=np.array([i * 30, 50]), confidence=0.9, embedding=embedding
            )
    ...
    def test_embedding_consistency_across_updates(self):
        ...
        for i in range(15):
            ...
            detection = Detection(
                position=np.array([10 + i, 20]), confidence=0.9, embedding=current_embedding
            )
    ...
    def test_embedding_scaling_numerical_stability(self):
        ...
        scaler = EmbeddingDistanceScaler(method="robust_minmax", min_samples=20)
    ...
    def test_embedding_memory_efficiency(self):
        ...
        base_embeddings = [
            np.random.randn(large_embedding_dim).astype(np.float32) for _ in range(5)
        ]
...
class TestEmbeddingScalingMethods:
    ...
    def test_scaling_method_properties(self, method, expected_properties):
        ...
        if expected_properties.get("preserves_order"):
            ...
            ), f"Order preservation failed: {order_similarity:.3f} similarity"
    ...
    def test_scaling_method_comparison(self):
        ...
        for i, method1 in enumerate(methods_list):
            for method2 in methods_list[i + 1 :]:
                ...
                assert (
                    correlation > 0.5
                ), f"{method1} vs {method2}: correlation = {correlation} (too low)"
                if correlation >= 0.999:
                    ...
                    assert (
                        correlation < 0.99
                    ), f"{method1} vs {method2}: correlation = {correlation} (identical methods?)"

    def test_scaling_adaptation_over_time(self):
        """Test how scaling adapts as more data arrives."""
        scaler = EmbeddingDistanceScaler(
            method="robust_minmax", update_rate=0.1, min_samples=30  # Faster adaptation for testing
        )
...
class TestEmbeddingIntegrationScenarios:
    ...
    def test_multi_class_embedding_tracking(self):
        ...
        for class_id in [0, 1, 2]:
            for instance in range(3):  # 3 instances per class
                obj = {
                    ...
                    "instance": instance,
                    "position": np.array(
                        [class_id * 80 + instance * 25, 50 + instance * 30], dtype=np.float32
                    ),
        ...
        for frame in range(25):
            ...
            for obj in objects:
                ...
                embedding = (
                    obj["embedding_template"]
                    + np.random.randn(128) * 0.1
                    + np.random.randn(128) * 0.05 * obj["instance"]  # General noise
                )  # Instance variation
    ...
    def test_embedding_scaling_with_diverse_data(self):
        ...
        if len(scaling_evolution) > 10:
            ...
            assert (
                later_stats["min_distance"] != early_stats["min_distance"]
                or later_stats["max_distance"] != early_stats["max_distance"]
                or (
                    later_stats["min_distance"] is not None
                    and later_stats["max_distance"] is not None
                )
            )
...
if __name__ == "__main__":
    ...
    scaled = scaler.scale_distances(distances)
    print(
        f"Scaler test: {len(scaled)} distances scaled, range: {np.min(scaled):.3f} - {np.max(scaled):.3f}"
    )
