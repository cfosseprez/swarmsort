...
class TestEdgeCaseInputs:
    ...
    def test_single_detection_sequences(self):
        ...
        for i in range(10):
            detection = Detection(
                position=np.array([10.0 + i, 20.0 + i * 0.5]),
                confidence=0.9
            )
    ...
    def test_invalid_bbox_values(self):
        ...
        invalid_bbox_detections = [
            Detection(position=np.array([10.0, 20.0]), confidence=0.8, bbox=np.array([0, 0, -10, -10])),  # Negative size
            Detection(position=np.array([30.0, 40.0]), confidence=0.9, bbox=np.array([100, 100, 50, 50])),  # x1 > x2, y1 > y2
            Detection(position=np.array([50.0, 60.0]), confidence=0.7, bbox=np.array([np.inf, 0, 10, 10])),  # Infinity
        ]
...
class TestHighLoadScenarios:
    ...
    def test_very_noisy_detections(self):
        ...
        config = SwarmSortConfig(
            max_distance=80.0,  # Larger distance to handle noise
    ...
    def test_very_long_sequence(self):
        ...
        config = SwarmSortConfig(
            max_age=20,
            max_embeddings_per_track=8  # Limit memory usage
        )
        tracker = SwarmSort(config)
        ...
        for i in range(num_objects):
            objects.append({
                ...
                'embedding': np.random.randn(64).astype(np.float32) if config.use_embeddings else None
            })
...
class TestResourceLimitTests:
    ...
    def test_concurrent_access_simulation(self):
        ...
        for i in range(10):
            detections = [
                Detection(position=np.array([i * 20 + j * 5, 50]), confidence=0.9)
                for j in range(3)
            ]
        ...
        for frame in range(num_frames):
            ...
            for i, obj in enumerate(objects):
                ...
                if obj['type'] == 'linear':
                    ...
                    obj['position'] = obj['center'] + obj['radius'] * np.array([
