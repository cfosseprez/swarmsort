...
from swarmsort import SwarmSortTracker, SwarmSortConfig, Detection, TrackedObject, SwarmSort
...
class TestCoreNumbaFunctions:
    ...
    def test_compute_embedding_distances_multi_history(self):
        ...
        track_counts = np.array(
            [3, 2, 3], dtype=np.int32
        )  # 3 tracks with different embedding counts
...
class TestTrackState:
    ...
    def test_track_state_with_bbox(self):
        ...
        track = FastTrackState(id=2, position=np.array([50.0, 100.0], dtype=np.float32), bbox=bbox)
